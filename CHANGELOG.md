...
- Added automatic bridge-mode fallback when the NanoGPT API rejects a native tool-calling request due to malformed tool-call data (HTTP 400). The new `isMalformedToolCallError()` detector catches this API-level rejection and retries the turn via the text-based JSON bridge contract instead of surfacing a hard failure to the user.
...
- Added model-specific tokenizer via `js-tiktoken` — `estimateTokenCount()` now accepts an optional `tokenizer` parameter and uses the BPE encoder matching the model's vocabulary (`cl100k_base` or `o200k_base`) for accurate token counts instead of the character-count heuristic. Falls back to the heuristic when no tokenizer hint is available. Uses a lazy encoder cache to avoid reconstructing expensive BPE rank tables.
- Added `truncateMessagesForContext()` - a pre-flight token-budget validator that estimates total message tokens and drops the oldest non-system messages when the count exceeds the model's reported `maxInputTokens`. Wired into both native and bridge streaming paths so the extension truncates context proactively instead of letting the upstream model silently truncate mid-stream, which was causing "model just stops with 0 tool calls" symptoms on long conversations. Abnormal finish reasons (`"length"` for truncation, `"content_filter"` for refusal) are now logged as warnings and exposed in `StreamProcessingSummary.finishReason` so callers can distinguish normal completions from truncated or refused responses.
